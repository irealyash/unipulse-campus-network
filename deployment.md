...
- Custom API subdomain: `api.unipulse.live` → Railway  
- Mongo Atlas automated backups (paid tier)  
- Cloudinary upload presets / size caps  
- Rate-limit auth routes (`/signup`, `/forgot-password`) to protect Resend quota  
- Separate staging Railway + Vercel preview env  

---

*Last updated for UniPulse launch prep. Always re-check provider dashboards for current free-tier numbers before relying on them.*
